...
test("accounts list and reconnect flows render the current control matrix", async ({
	...
	await expect(reconnectRow).toContainText("needs_reconnect");
	await expect(
	...
	await expect(page.getByRole("heading", { name: "PW Backlog Gmail" })).toBeVisible();
	await expect(page.getByText("Paused", { exact: true })).toBeVisible();
	await expect(page.getByRole("button", { name: "Resume" })).toBeVisible();
	await expect(
		page.getByRole("button", { name: "Disconnect Gmail" }),
	).toBeVisible();
	await expect(page.getByRole("link", { name: "Delete local account" })).toBeVisible();
...
async function waitForAccountsRowGone(page: import("@playwright/test").Page, label: string) {
	await expect
		.poll(
			async () =>
				page
