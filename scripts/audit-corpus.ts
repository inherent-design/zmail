...
import { runtimePaths, runWithOrgContext } from "#/lib/runtime";
...
function parseArgs(argv = process.argv): ParsedArgs {
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		throw new Error(`Unsupported audit:corpus arg: ${arg}`);
	}

	if (!orgId) {
		throw new Error("audit:corpus requires --org <orgId>");
	}
...
export async function main(_trace?: LogTrace, argv = process.argv) {
	...
	await runWithOrgContext(args.orgId, async () => {
		...
		const runtime = runtimePaths(args.orgId);
		const resolvedConfig = loadResolvedConfig();
		const externalFinanceImportPath =
			resolvedConfig.server.basePath === "/"
				? "/api/finance/imports"
				: `${resolvedConfig.server.basePath}/api/finance/imports`;
		...
		for (const row of moderationRows) {
			const promptVersion = getStoredModerationPromptVersion(
		...
		const openJobCounts = jobRows.reduce<
		...
		const report = {
			...
			auth: {
				financeImportBrowserRoles: ["org_admin", "org_operator"],
				financeImportRequiresOrgScopedMachineToken: true,
				externalFinanceImportPath,
			},
			financeImportDedup: {
				importRuns: count("SELECT COUNT(*) AS count FROM finance_import_runs"),
				distinctArtifacts: count(
					"SELECT COUNT(DISTINCT artifact_sha256) AS count FROM finance_import_runs",
				),
				duplicateArtifactRows: count(
					`
